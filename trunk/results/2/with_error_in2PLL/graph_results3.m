...
if Profile == 1
    ...
    JS_K2PLL = 30:0.1:45; EK2PLL = interp1(JS_K2PLL1, EK2PLL, JS_K2PLL, 'cubic');    
end
...
cla
hold on
p = plot(SFS.JS_ArgMax, SFS.EArgMax*360, '.', JS_ArgMax, EArgMax, 'LineWidth', 2);  set(p, 'Color', [0, 0, 1]);
p = plot(SFS.JS_Mean, SFS.EMean*360, '.', JS_Mean, EMean,  'LineWidth', 2); set(p, 'Color', [0.0, 0.5, 0.8]);
p = plot(SFS.JS_BigKalm2_w0, SFS.EBigKalm2_w0*360, '.', JS_BigKalm2_w0, EBigKalm2_w0, 'LineWidth', 2); set(p, 'Color', [0.9, 0, 0]);
p = plot(SFS.JS_Kalm2_w0, SFS.EKalm2_w0*360, '.', JS_Kalm2_w0, EKalm2_w0, 'LineWidth', 2); set(p, 'Color', [0, 0.9, 0]);
p = plot(SFS.JS_K2PLL, SFS.EK2PLL*360, '.', JS_K2PLL, EK2PLL, 'LineWidth', 2); set(p, 'Color', [0.5, 0.5, 0.5]);
hold off
